class Address
  ...
  # modes, as described below.
    ...
    :zp, # (zero page) In this mode the operands address is given.
    ...
    :rel, # (relative) This mode is used with Branch-on-Condition instructions.
    ...
    :imp, # (implied) No operand addresses are required for this mode. They are
    # implied by the instruction.
    # eg.  TAX - (transfer accumulator contents to index x)
    #      0xAA

    :abs, # (absolute) In this mode the operands address is given.
    # eg.  LDA  0x31F6 - (assembler)
    #      0xAD 0x31F6 - (machine code)
    # Note that for 2 byte addresses, the low byte is store first, eg.
    # LDA 0x31F6 is stored as three bytes in memory, 0xAD 0xF6 0x31.

    :acc, # (accumulator) In this mode the instruction operates on data in the
    # accumulator, so no operands are needed.
    # eg.  LSR - logical bit shift right
    #      0x4A

    :imm, # (immediate) In this mode the operand's value is given in the instruction
    ...
    :zpx, :zpy, :absx, :absy, # (zero page x/y and absolute x/y) In these modes
    ...
    :preind, # (preindexed indirect) In this mode a zer0-page address is added
    ...
    :postind, # (postindexed indirect) In this mode the contents of a zero-page address
    ...
    :absind  # (absolute indirect) This mode applies only to the JMP instruction
